...
def analyze_each_experiment():
    for application in configs.APPLICATIONS:
        with open('results3/latency/summaries/' + application + '-raspberrypi-no-fault-summary.csv', 'w',
                  encoding='UTF8',
            ...
            while experiment_id <= configs.REPEAT_EXPERIMENTS:
                ...
                result_latency_filename = 'results3/latency/' + application + '/raspberrypi-no-fault-exp' + str(
                    experiment_id) + ".csv"
        ...
        for fault in configs.FAULTS:
            for fault_config in fault.fault_config:
                with open(
                        'results3/latency/summaries/' + application + '-raspberrypi-' + fault.abbreviation + '-' + fault_config + '-summary.csv',
                        'w', encoding='UTF8',
                        newline='') as csv_output:
                    writer = csv.writer(csv_output)
                    experiment_id = 1
